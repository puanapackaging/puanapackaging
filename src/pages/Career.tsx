...
// Define interfaces for form data and job openings
interface FormData {
  name: string;
  email: string;
  phone: string;
...
const Careers: React.FC = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value, files } = e.target as HTMLInputElement; // Type assertion for file input
    setFormData((prev) => ({
      ...prev,
      [name]: files ? files[0] : value
    }));
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header />
      <section id="careers" className="py-20 bg-gradient-to-b from-gray-50 to-white flex-1">
        {/* Banner */}
        <div className="relative h-96 mb-16 overflow-hidden">
          <img
            src="https://images.unsplash.com/photo-1521737604893-d14cc237f11d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80"
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-primary/80 via-primary/60 to-primary/30 flex items-center justify-center">
            <div className="text-center text-white px-4">
              <h1 className="text-4xl md:text-6xl font-bold mb-4 drop-shadow-lg">Join Our Team</h1>
        ...
        <div className="container mx-auto px-4">
          ...
          {/* Job Application Form */}
          <div className="bg-white rounded-xl p-8 shadow-xl border border-secondary/20 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-br from-secondary/10 to-transparent transform rotate-12 translate-x-1/3" aria-hidden="true" />
            <div className="relative">
              <div className="flex items-center mb-8">
                <h3 className="text-2xl md:text-3xl font-bold text-primary flex items-center">
                  <FaBriefcase className="mr-3 text-accent" aria-hidden="true" /> Apply Now
                </h3>
              </div>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  {/* Name */}
                  <div className="relative">
                    <FaUser className="absolute top-3 left-3 text-gray-400" aria-hidden="true" />
                    <input
                      type="text"
                      name="name"
                      value={formData.name}
                  ...
                  {/* Email */}
                  <div className="relative">
                    <FaEnvelope className="absolute top-3 left-3 text-gray-400" aria-hidden="true" />
                    <input
                      type="email"
                      name="email"
                      value={formData.email}
                  ...
                  {/* Phone */}
                  <div className="relative">
                    <FaPhone className="absolute top-3 left-3 text-gray-400" aria-hidden="true" />
                    <input
                      type="tel"
                      name="phone"
                      value={formData.phone}
                  ...
                  {/* Position */}
                  <div className="relative">
                    <FaBriefcase className="absolute top-3 left-3 text-gray-400" aria-hidden="true" />
                    <select
                      name="position"
                      value={formData.position}
                      onChange={handleChange}
                ...
                {/* Resume Upload */}
                <div className="relative">
                  <FaFileUpload className="absolute top-3 left-3 text-gray-400" aria-hidden="true" />
                  <input
                    type="file"
                    name="resume"
                    onChange={handleChange}
                ...
                >
                  Submit Application
                  <FaPaperPlane className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" aria-hidden="true" />
                </button>
              </form>
            </div>
          </div>
          ...
          {/* Open Positions Teaser */}
          <div className="mt-16 text-center">
            <h3 className="text-xl md:text-2xl font-bold text-primary mb-4">Current Openings</h3>
            <p className="text-gray-700 mb-6 max-w-2xl mx-auto">
              Check out some of our available positions and apply today!
            </p>
            <div className="grid md:grid-cols-3 gap-6">
              {jobOpenings.map((job) => (
                ...
                >
                  <h4 className="text-lg md:text-xl font-semibold text-primary">{job.title}</h4>
                  <p className="text-gray-600 mt-2">{job.desc}</p>
                </div>
              ))}
            </div>
            <button 
              className="mt-8 flex items-center mx-auto text-accent hover:text-primary font-semibold transition-colors group"
              aria-label="View All Positions"
            >
              View All Positions
              <FaArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-2 transition-transform" aria-hidden="true" />
            </button>
          </div>
        </div>
      </section>
